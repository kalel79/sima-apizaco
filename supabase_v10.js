...
export async function getIndicadoresLista() {
  // Traer en 2 páginas de 100 para superar el límite del proyecto Supabase
  const [
    { data: p1, error: e1 },
    { data: p2, error: e2 },
    { data: p3, error: e3 },
    ...
    supabase.from('indicadores').select('id, nombre, nivel_mir, area_id').order('nombre').range(0,   99),
    supabase.from('indicadores').select('id, nombre, nivel_mir, area_id').order('nombre').range(100, 199),
    supabase.from('indicadores').select('id, nombre, nivel_mir, area_id').order('nombre').range(200, 299),
